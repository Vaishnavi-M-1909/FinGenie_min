...
def extract_text(uploaded_file, force_ocr: bool = False, password: str = None):
    ...
    # 1) pdfplumber (supports password)
    try:
    ...
    if (force_ocr) and convert_from_bytes and pytesseract:
        dec_bytes = None
        ...
        try:
            ...
            text = "\n".join([pytesseract.image_to_string(img, config="--oem 1 --psm 6") for img in images])
            if text.strip():
    ...
    diag["note"] = (diag.get("note", "") + " | no text detected; check OCR or password").strip()
    return "", diag

# ---------- bank detection ----------
def detect_bank(text: str) -> str:
    t = text.lower()
    if "kotak mahindra bank" in t or ("withdrawal amt" in t and "deposit amt" in t and "balance" in t):
        return "kotak"
    return "generic"
...
def parse_kotak_statement(text: str) -> pd.DataFrame:
    ...
    for blk in blocks:
        ...
        if txn_amount is None:
            try: txn_amount = float(bal_token.replace(",",""))
            except: continue
        ...
        if re.search(r"\bdr\b|\bdebit\b|\bwithdraw", low): ttype = "Debit"
...
def parse_text_generic(text: str) -> pd.DataFrame:
    ...
    for ln in lines:
        ...
        rows.append({"Date": safe_date(date_str), "Description": desc if desc else "—", "Amount": abs(amount), "Type": ttype})
    return pd.DataFrame(rows, columns=["Date","Description","Amount","Type"])
...
def compute_basic_stats(df):
    if df.empty: return {"n_txn":0,"sum_debits":0.0,"sum_credits":0.0}
...
def categorize_transactions(df):
    """Rule-based categorization on Debits; Credits kept as Income."""
    if df.empty:
        return df
    df = df.copy()
    df["Category"] = "Other"
    # Only apply categories to debits (spends)
    debit_mask = df["Type"].str.lower().eq("debit")

    rules = {
        r"zomato|swiggy|domino": "Food",
        r"uber|ola|rapido": "Transport",
        r"amazon|flipkart|myntra": "Shopping",
        r"petrol|hpcl|bpcl|ioc": "Fuel",
        r"atm|withdraw|cash": "Cash Withdrawal",
        r"emi|loan|nbfc": "Loan/EMI",
        r"rent|landlord|lease": "Rent",
        r"electric|mseb|tata power|bills|gas": "Utilities",
        r"movie|bookmyshow|pvr|inox": "Entertainment",
        r"medical|pharma|apollo|practo|hospital": "Healthcare",
    }
    desc = df["Description"].str.lower().fillna("")
    for pat, cat in rules.items():
        df.loc[debit_mask & desc.str.contains(pat, regex=True), "Category"] = cat

    # credits → Income
    df.loc[df["Type"].str.lower().eq("credit"), "Category"] = "Income"
    return df

def summarise_categories(df):
    if df.empty:
        return (None, 0.0), (None, 0.0)
    spends = df[df["Category"] != "Income"]  # only spending categories
    if spends.empty:
        return (None, 0.0), (None, 0.0)
    s = spends.groupby("Category")["Amount"].sum().sort_values(ascending=False)

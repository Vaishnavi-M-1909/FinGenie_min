...
# Optional fallback import for tricky PDFs
try:
    import fitz  # PyMuPDF
except Exception:
    fitz = None
...
# ---------- public entry ----------
def process_pdf_to_df(uploaded_file, return_text: bool = False):
...
def extract_text(uploaded_file):
    """Try multiple engines. Return (text, diagnostics)."""
    ...
    # 3) none (likely scanned)
    diag["engine"] = diag.get("engine") or "none"
    diag["note"] = (diag.get("note", "") + " | likely scanned/image-only; needs OCR").strip()
    return "", diag
...
def parse_kotak_statement(text: str) -> pd.DataFrame:
    """
    Typical Kotak columns:
    Date | Narration | Chq/Ref No. | Withdrawal Amt. | Deposit Amt. | Balance
    Strategy:
    - Build row blocks that start with a date at the beginning of the line.
    - For each block, pick the last numeric as Balance.
    - From remaining numeric tokens, pick the rightmost non-zero as transaction amount.
    - CR/DR words tilt classification; else use which side appears populated.
    """
    ...
    for blk in blocks:
        ...
        # flatten one-line version for numeric scan
        one = " ".join(ln.strip() for ln in blk.splitlines())
        # amounts (ignore ref numbers by being strict: must have comma groups or decimal)
        ...
        # last number is typically Balance
        bal_token = nums[-1]
        tail = nums[:-1] if len(nums) > 1 else []

        # choose txn amount as rightmost non-zero from tail
        txn_amount = None
        ...
        # Narration: remove date + trailing numeric tail
        date_union = "(" + "|".join(DATE_PATTERNS) + ")"
        narration = re.sub(date_union, "", one).strip()
        # strip trailing amount tokens / CRDR at the end
...
def parse_text_generic(text: str) -> pd.DataFrame:
    ...
    for ln in lines:
        ...
        ttype = "Credit" if (marker and marker.group(0).lower().startswith("cr")) or " credit" in ln.lower() or amount > 0 else "Debit"
        rows.append({
            "Date": safe_date(date_str),
